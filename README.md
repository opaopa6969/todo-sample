...
- 認証・ユーザー管理・セッション管理をこのアプリや BaaS に取り込むこと。認証境界は `volta-auth-proxy` に置き、このアプリは信頼できる前段から渡された識別ヘッダだけを扱う。
- BaaS 固有の SDK やデータモデルへ依存すること。教材の焦点を認証境界と tenant × user 分離に保つため、ストレージは意図的にインメモリのままとする。
- 本番向けの永続化・可用性・運用機能を提供すること。このリポジトリは認証境界を学ぶための最小サンプルであり、汎用的な todo サービスを目指さない。
